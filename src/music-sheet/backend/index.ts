...
import { getMediaPrimaryKey, isSameMedia } from "@/common/media-util.ts";
import {
  getUserPreferenceIDB,
  setUserPreferenceIDB,
} from "@/utils/user-perference.ts";
  ...
  try {
    ...
    await musicSheetDB.transaction(
      ...
      async () => {
        ...
        ) as (IMusic.IMusicItem & { [musicRefSymbol]: number })[];
  ...
  try {
    await musicSheetDB.transaction(
      ...
      async () => {
        ...
        // 更新当前歌单的封面
        let newArtwork: string | undefined;
        if (lastMusic) {
          newArtwork = (
    ...
    async () => {
      ...
      const allSheetDetails = produce(allSheets, (draft) => {
        draft.forEach((sheet, index) => {
          sheet.musicList = musicLists[index] as IMusic.IMusicItem[];
        });
      });
